...
GroupCommand::GroupCommand(size_t number_of_modules)
    : internal_(amberGroupCommandCreate(number_of_modules)), number_of_modules_(number_of_modules) {
...
AmberCommandPtr GroupCommand::operator[](size_t index) { return commands_[index]; }

const AmberCommandPtr GroupCommand::operator[](size_t index) const { return commands_[index]; }
...
void GroupCommand::setMontionCtrlConfig(const std::vector<MotionControllerConfig*> &config)
{
