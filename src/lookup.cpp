...
{
    return amberLookupGetCtrlBoxIP(lookup_);
}

std::shared_ptr<Group> Lookup::getGroupFromFamily(
    const std::string& family, 
    int32_t timeout_ms) 
{
  std::shared_ptr<Group> ptr;
  AmberGroupPtr group = amberGroupCreateFromFamily(lookup_, family.c_str(), timeout_ms);
  if (group != nullptr)
    return std::make_shared<Group>(group);
  return ptr;
}

float Lookup::getLookupFrequencyHz() const { return amberLookupGetLookupFrequencyHz(lookup_); }

bool Lookup::setLookupFrequencyHz(float frequency) 
{
  return amberLookupSetLookupFrequencyHz(lookup_, frequency) == AmberStatusSuccess;
}
...
Lookup::EntryList::Entry Lookup::EntryList::operator[](size_t index) const
{
    ...
    amberLookupEntryListGetSerialNumber(lookup_list_, index, serial_number_, &len);
